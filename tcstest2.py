...
#----------------------------------------I/O pins---------------------------------------------------
allowReadPin = machine.Pin(0, Pin.IN, None)
allowReadPin.irq(trigger = Pin.IRQ_RISING, handler = allowRead_callback)
allowReadPin.irq(trigger = Pin.IRQ_FALLING, handler = disableRead_callback)
allowRead = False

def allowRead_callback:
    allowRead = True

def disableRead_callback:
    allowRead = False
#----------------------------------------WiFi setup-------------------------------------------------
labNetwork = 'EEERover'
...
#----------------------------------------Data reading-----------------------------------------------
while allowRead == True:
    s = sensor.read(True)
    s = convert_rgb_data(s)
    #print(s)
    client.publish('esys/TBA/sensor', 'RGBC: (' 
            + str(s[1]) + ', '+ str(s[1]) + ', ' + str(s[2]) + ', ' + str(s[3]) +')')
    time.sleep_ms(100)
    client.wait_msg()
